...
export default function ResetPassword() {
  ...
  function confirmPasswordChange(e){
   ...
   if (!token || !userId) {
    ...
    try {
      const response = await fetch(`${serverBaseUrl}/users/v1/reset-password`, {
        method: 'POST',
      ...
      if (response.ok && result.success) {
        ...
        setStatus({
          loading: false,
          error: result.message || 'Failed to reset password. Link may have expired.',
          success: '',
        });
      }
    } catch (err) {
      ...
      <div className="w-full max-w-6xl h-screen sm:h-[850px] bg-[#12141c] sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row border border-gray-800/50">
        ...
        <div className="hidden md:flex md:w-1/2 relative bg-cover bg-center items-end p-12" 
            ...
            <div className="space-y-4 max-w-md z-10">
                ...
                <p className="text-gray-400 text-sm leading-relaxed">Create your new password here And login using the new password!</p>
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center px-6 py-12 sm:px-16 lg:px-20 bg-[#12141c]">
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-400 text-sm mt-2">Please enter your new Password to securely gain access to you account</p>
            </div>
            ...
            <form className="space-y-5" >
                <div className="space-y-2">
                    <label htmlFor="newpassword" className="text-xs font-semibold text-gray-400 uppercase tracking-wider">New Password</label>
                    <div className="relative">
                        ...
                        <input type="password" id="newpassword" required
                               ...
                               placeholder="••••••••" value={newPassword} onChange={newPasswordChange}/>
                    </div>
                </div>
                 <div className="space-y-2">
                    <label htmlFor="confirmpassword" className="text-xs font-semibold text-gray-400 uppercase tracking-wider">New Password</label>
                    <div className="relative">
                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-500">
                            <i data-lucide="mail" className="w-5 h-5"></i>
                        </div>
                        <input type="password" id="confirmpassword" required
                               className="w-full pl-11 pr-4 py-3.5 bg-[#1a1d26] border border-gray-800 rounded-xl text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition duration-200" 
                               placeholder="••••••••" value={confirmPassword} onChange={confirmPasswordChange}/>
                    </div>
                </div>
        ...
        {status.success && (
          <div className="mb-4 p-3 bg-emerald-500/10 border border-emerald-500/50 text-emerald-400 text-sm rounded-lg text-center">
            {status.success}
          </div>
        )}
